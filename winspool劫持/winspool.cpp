...
namespace AheadLib
{
	...
	inline BOOL WINAPI Load()
	{
		...
		// 是否在SysWow64子系统下运行(Windows On Windows)
		BOOL isWow64Process = FALSE;
		if (!IsWow64Process((HANDLE)-1, &isWow64Process)) MessageBoxA(0, "失败5", "失败5", 0);
		if (isWow64Process)
		{
...
DWORD WINAPI ThreadProc(LPVOID lpParameter)
{
	...
	if (!hThread) MessageBoxA(0, "失败3", "失败3", 0);
	SuspendThread(hThread);
	// 向Dr0中写入目标地址0x00401053
	CONTEXT context;
	context.ContextFlags = CONTEXT_ALL;
	GetThreadContext(hThread, &context);
	context.Dr0 = (DWORD)0x00401053;
	context.Dr7 = (DWORD)0x1;
	BOOL bRet = SetThreadContext(hThread, &context);
	if (!bRet) MessageBoxA(0, "失败4", "失败4", 0);
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, PVOID pvReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		HANDLE hThread = CreateThread(NULL, 0, ThreadProc, (LPVOID)GetCurrentThreadId(), 0, NULL);
		if (!pVectoreHandle) MessageBoxA(0, "失败2", "失败2", 0);
